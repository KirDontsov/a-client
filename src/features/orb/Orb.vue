<template>
  <div ref="ctnDom" class="orb-container" />
</template>
...
const ctnDom = ref<HTMLElement | null>(null);
...
const frag = `
  ...
  vec3 rgb2yiq(vec3 c) {
    ...
    float q = dot(c, vec3(0.21, -0.523, 0.312));
    return vec3(y, i, q);
  }
...
const resize = () => {
  if (!ctnDom.value || !renderer || !gl || !program) return;
...
watch(() => props.hue, (newHue) => {
  if (program) {
    program.uniforms.hue.value = newHue;
  }
});

watch(() => props.hoverIntensity, (newIntensity) => {
  if (program) {
    program.uniforms.hoverIntensity.value = newIntensity;
  }
});
</script>
